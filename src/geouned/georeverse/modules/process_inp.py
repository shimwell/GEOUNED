...
def set_options(optionFile):
    ...
    for section in config.sections():
        ...
        if section == "Setting":
            fileIn, fileOut, outBox, inFormat = get_setting(config)
            ...
            UStart, levMax = get_level(config)
            ...
            CMRange = get_range(section, config)
            if CMRange is None:
                continue
            if section == "Cells":
...
def get_setting(config):
    ...
    for key in config["Setting"].keys():
        ...
        if key in ["inputFile", "mcnpFile"]:
            ...
            outBox = get_box_data(config.get("Setting", key))
        elif key == "inFormat":
...
def get_range(section, config):
    ...
    if rType is None:
        ...
        ctRange = get_range_data(rawRange)
        if ctRange is None:
