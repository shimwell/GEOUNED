...
def split_solid(base, surfacesCut, cellObj, solidTool=False, tolerance=0.01):  # 1e-2
    ...
    for pos, sol in zip(partPositions, partSolids):
        # fullPos = update_surfaces_values(pos,cellObj.surfaces,base.knownSurf)
...
def update_surfaces_values(position, surfaces, knownSurf):
...
def surface_side(p, surf):
    if surf.type == "sphere":
        ...
        if not surf.truncated:
            ...
            inPln = btw_p_planes(p, P, v)  # <0  between planes
        ...
        if surf.truncated and inout < 0:
            inout = btw_p_planes(p, center, axis)  # <0  between planes
        ...
        for v in (v1, v2, v3):
            inout = btw_p_planes(p, P, v)  # <0  between planes
            if inout > 0:
                break
