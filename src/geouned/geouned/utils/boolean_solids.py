...
from .functions import GEOUNED_Surface, splitBOP
from .options.classes import Options as opt
...
def build_c_table_from_solids(Box, SurfInfo, option="diag"):
    ...
    if type(surfaces[surfaceList[0]]) is GEOUNED_Surface:
        ...
        for s in surfaceList:
            surfaces[s].build_shape(Box.BoundBox)
    ...
    for subCell in CellSeq.elements:
        ...
        if not nullcell:
            ...
            if res == None:
                ...
                if subCell.level == 0:
                    remove_surf = outSurfaces & subCell.get_surfaces_numbers()
...
def split_solid_fast(solid, surf, box):
    ...
    if box:
        if surf.shape:
            comsolid = splitBOP(solid, [surf.shape], opt.split_tolerance)
        else:
        ...
        if surf.shape:
            dist = solid.dist_to_shape(surf.shape)[0]
        else:
            dist = 1.0
