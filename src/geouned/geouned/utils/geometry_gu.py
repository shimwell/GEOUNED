...
from .options.classes import Tolerances as tol
...
class SolidGu:
        ...
        for tset in faceList:
            ...
            while len(temp) > 0:
                ...
                while len(temp) > 0 and i < len(current):
                    for tindex in temp:
                        if (
                            self.Faces[current[i]].dist_to_shape(self.Faces[tindex])[0]
                            < tol.distance
                        ):
    ...
    def __mergePeriodicUV__(self, parameter, faceList):
        ...
        V0 = params[0][0]
        V1 = params[-1][1]
        if arcLength >= twoPi * (1.0 - tol.relative_precision):
            mergedParams = (True, (V0, V0 + twoPi))
        else:
            if is_same_value(V0, 0.0, tol.relative_precision) and is_same_value(
                V1, twoPi, tol.relative_precision
            ):
                for i in range(len(params) - 1):
                    if not is_same_value(
                        params[i][1], params[i + 1][0], tol.relative_precision
                    ):
                        break
...
def define_list_face_gu(face_list, plane3Pts=False):
    ...
    return tuple(FacesGu(face, plane3Pts) for face in face_list)
