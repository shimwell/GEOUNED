...
def is_opposite(Vector1, Vector2, tolerance=1e-6):
    return abs(Vector1.getAngle(-Vector2)) < tolerance


def is_parallel(Vector1, Vector2, tolerance=1e-6):
...
def is_in_edge(Edge1, Edge2, tolerance=1e-8):
    Ver1 = Edge1.Vertexes
    Ver2 = Edge2.Vertexes
    con1 = Ver1[0].Point.is_equal(Ver2[0].Point, tolerance) or Ver1[0].Point.is_equal(
        Ver2[1].Point, tolerance
    )
    con2 = Ver1[1].Point.is_equal(Ver2[0].Point, tolerance) or Ver1[1].Point.is_equal(
        Ver2[1].Point, tolerance
    )
    return con1 and con2


def is_in_plane(Point, plane, dtolerance=1e-7):
    return abs(Point.distanceToPlane(plane.Surf.Position, plane.Surf.Axis)) < dtolerance


def is_in_tolerance(val, tol, fuzzyLow, fuzzyHigh):
    if abs(val) < fuzzyLow:
