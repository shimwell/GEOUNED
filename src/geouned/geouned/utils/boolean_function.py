...
class BoolSequence:
    ...
    def simplify_sequence(self, CT):
        ...
        for valname in surfNames:
            if valname in newNames:
                ...
                if not self.do_factorize(valname, trueSet, falseSet):
                    continue
    ...
    def do_factorize(self, valname, trueSet, falseSet):
        ...
        valSet = self.get_surfaces_numbers()
    ...
    def substitute(self, var, val):
        ...
        self.join_operators(selfLevel=True)
    ...
    def join_operators(self, selfLevel=False):
        if type(self.elements) is bool:
            return
        self.clean(selfLevel=True)
        self.level_update()
    ...
    def get_sub_sequence(self, setIn):
        ...
        if len(position) == 1 and subSeq.elements[0].level > 0:
            subList, subSeq = subSeq.elements[0].get_sub_sequence(valSet)
    ...
    def factorize(self, valname, trueSet, falseSet):
        ...
        pos, subSeq = self.get_sub_sequence(valSet)
        updt = True
        if len(pos) == 0:
            subSeq = self
            updt = False
        ...
        if trueFunc == False:
            ...
            if falseFunc == True:
                ...
                newSeq.join_operators(selfLevel=True)
