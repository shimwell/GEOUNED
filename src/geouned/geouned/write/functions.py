...
def write_mcnp_cell_def(Definition, tabspace=0, offset=0):
    ...
    strDef = remove_redundant(write_sequence_mcnp(Definition))
    sdef.add(strDef)
...
def write_serpent_cell_def(Definition, tabspace=0, offset=0):
    sdef = CellString(tabspace=tabspace)
    strDef = remove_redundant(write_sequence_serpent(Definition))
    sdef.add(strDef)
    sdef.wrapLine(offset)
    return sdef.str
...
def write_phits_cell_def(Definition, tabspace=0, offset=0):
    sdef = CellString(tabspace=tabspace)
    strDef = remove_redundant(write_sequence_phits(Definition))
    sdef.add(strDef)
    sdef.wrapLine(offset)
    return sdef.str


def write_openmc_region(Definition, Wtype="XML"):
    if Wtype == "XML":
        return write_sequence_openmc_xml(Definition)
    if Wtype == "PY":
        return write_sequence_openmc_py(Definition)
...
def write_sequence_openmc_xml(Seq):
    if Seq.level == 0:
        if Seq.operator == "AND":
            line = "({})".format(" ".join(map(str, Seq.elements)))
        else:
        ...
        for e in Seq.elements:
            if type(e) is int:
                ...
                terms.append(write_sequence_openmc_xml(e))
...
def write_sequence_openmc_py(Seq, prefix="S"):
    ...
    if Seq.level == 0:
        ...
        for e in Seq.elements:
            if type(e) is int:
                ...
                terms.append(write_sequence_openmc_py(e))
...
def mcnp_surface(id, Type, surf):
    ...
    if Type == "Plane":
        ...
        Pos = surf.Center * 0.1
        rad = surf.Radius * 0.1
        if is_parallel(Dir, FreeCAD.Vector(1, 0, 0), tol.angle):
        ...
        elif is_parallel(Dir, FreeCAD.Vector(0, 1, 0), tol.angle):
        ...
        elif is_parallel(Dir, FreeCAD.Vector(0, 0, 1), tol.angle):
...
{:<6d} GQ  {v[0]:{aTof}} {v[1]:{aTof}} {v[2]:{aTof}}
        ...
        if is_parallel(Dir, X_dir, tol.angle):
            sheet = 1
            if is_opposite(Dir, X_dir, tol.angle):
                sheet = -1
        ...
        elif is_parallel(Dir, Y_dir, tol.angle):
            sheet = 1
            if is_opposite(Dir, Y_dir, tol.angle):
                sheet = -1
        ...
        elif is_parallel(Dir, Z_dir, tol.angle):
            sheet = 1
            if is_opposite(Dir, Z_dir, tol.angle):
                sheet = -1
...
def openmc_surface(Type, surf, outXML=True, quadricForm=False):
    if Type == "Plane":
        A = surf.Axis.x
        B = surf.Axis.y
        C = surf.Axis.z
...
def SerpentSurface(id, Type, surf):
    ...
    if Type == "Plane":
        if surf.pointDef and opt.prnt3PPlane:
            ...
            C = surf.Axis.z
            D = surf.Axis.dot(surf.Position)
            if surf.Axis.is_equal(FreeCAD.Vector(1, 0, 0), tol.pln_angle):
                Serpent_def = f"surf {id} px {D/10:{nf.P_xyz}}"
            elif surf.Axis.is_equal(FreeCAD.Vector(0, 1, 0), tol.pln_angle):
                Serpent_def = f"surf {id} py {D/10:{nf.P_xyz}}"
            elif surf.Axis.is_equal(FreeCAD.Vector(0, 0, 1), tol.pln_angle):
