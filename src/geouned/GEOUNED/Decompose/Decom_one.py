...
def GenPlaneCylinder(face, solid, tolerances):
    ...
    if len(Uval_str_cl) < 2:
        if opt.verbose:
            print("GenPlaneCylinder : Uval_str_cl should no be void ")
        return None
...
def GenPlaneCone(face, solid, tolerances):
    ...
    if len(Uval_str_cl) < 2:
        if opt.verbose:
            print("GenPlaneCone : Uval_str_cl should no be void ")
        return None
...
def removeSolids(Solids):
    ...
    for solid in Solids:
        if solid.Volume <= Vol_tol:
            if opt.verbose:
                print(
                    "Warning: removeSolids degenerated solids are produced",
                    solid.Volume,
                )
            err = 2
            continue
...
def splitComponent(solidShape, UniverseBox, tolerances):
    ...
    for part in split:
        if part.Volume <= 1e-10:
            if opt.verbose:
                print(
                    "Warning: splitComponent degenerated solids are produced",
                    part.Volume,
                )
            err += 2
            continue
