...
class CadToCsg:
    ...
    def __init__(
        ...
        dummyMat: bool = False,
        volCARD: bool = True,
        UCARD: typing.Union[None, str] = None,
        simplify: str = "No",
        cellRange=[],
        exportSolids: str = "",
    ...
    def from_config_ini(cls, configFile):
        ...
        for section in config.sections():
            if section == "Files":
                ...
                PdEntry = False
                for key in config["NumericFormat"].keys():
                    if key in attributes_and_types.keys():
                        value = config.get("NumericFormat", key)
                        setattr(numeric_format_from_config, key, value)
                        print('setting', key, value)
                        # todo see if we can recode so we don't need this special treatment of the P_d key. 
                        if key == "P_d":
                            PdEntry = True
